...
class mcts():
	...
	def search(self, initialState, playerTrees):
		self.root = treeNode(initialState, None)
		playerTrees[initialState.current_player] = self.root
	...
	def search1(self, initialState, playerTrees):
		self.root = playerTrees[initialState.current_player]
...
#implementing MCTS with Poker
class PokerState():
	def __init__(self, current_player, is_bet_one, is_round_one, last_player_to_raise, amount_to_match, player_order):
	...
	def takeAction(self, action):
		...
		if action == 'Raise':
			newState.last_player_to_raise = self.betting_temp_player_list[self.player_order]['Name']
			newState.betting_temp_player_list[self.player_order]['Raised?'] = True
			newState.amount_to_match += newState.raise_amount
			newState.betting_temp_player_list[self.player_order]['Money'] += newState.amount_to_match - newState.betting_temp_player_list[self.player_order]['Money']
			newState.pot += newState.betting_temp_player_list[self.player_order]['Money'] - self.betting_temp_player_list[self.player_order]['Money']
		elif action == 'Check':
			newState.betting_temp_player_list[self.player_order]['Money'] += 0
		elif action == 'Call':
			newState.betting_temp_player_list[self.player_order]['Money'] += newState.amount_to_match - newState.betting_temp_player_list[self.player_order]['Money']
			newState.pot += (newState.pot - self.betting_temp_player_list[self.player_order]['Money']) + newState.betting_temp_player_list[self.player_order]['Money']
		elif action == 'Fold':
			newState.betting_temp_player_list[self.player_order]['Folded?'] = True
			newState.folded_players.append(newState.betting_temp_player_list[self.player_order]['Name'])
			
		if (newState.player_order) + 1 > len(newState.betting_temp_player_list) - 1:
			newState.player_order = 0
			newState.is_round_one = False
		else:
			newState.player_order += 1

		while (newState.betting_temp_player_list[newState.player_order]['Folded?']):
			if (newState.player_order) + 1 > len(newState.betting_temp_player_list) - 1:
				newState.player_order = 0
				newState.is_round_one = False
			else:
				newState.player_order += 1
		newState.current_player = newState.betting_temp_player_list[newState.player_order]['Name']
		return newState
		...
		if (self.is_round_one):
			...
			for num in range(0, len(self.betting_temp_player_list)):
				if not(self.betting_temp_player_list[num]['Folded?']):
					last_order = self.betting_temp_player_list[num]['Order']
			if (self.last_player_to_raise == '') and (self.player_order == last_order):
				return True
			else:
				return False
		else:
			if (self.betting_temp_player_list[self.player_order]['Name'] == self.last_player_to_raise):
				return True
			else:
				return False
	
	def getReward(self):
		return self.pot - self.betting_temp_player_list[self.player_order]['Money']
	...
	for player in betting_temp_player_list:
		if one_player_left == True:
			...
			if player['Player_Type'] != 'Human':
				if first_hand_yes:
					playerTrees[player['Name']] = 0
				initialState = PokerState(player['Name'], True, True, last_player_to_raise, amount_to_match, player['Order'])
				mcts1 = mcts(iterationLimit=ITER_LIM)

				#bestAction = mcts1.search1(initialState=initialState, playerTrees=playerTrees)
				if (first_hand_yes):
					bestAction = mcts1.search(initialState=initialState, playerTrees=playerTrees)
				else:
					bestAction = mcts1.search1(initialState=initialState, playerTrees=playerTrees)
				player['Decision'] = bestAction
		...
		if one_player_left == True:
			...
			if last_player_to_raise == '':
				...
				if player['Name'] == last_player_to_raise:
					...
					if player['Player_Type'] != 'Human':
						initialState = PokerState(player['Name'], True, False, last_player_to_raise, amount_to_match, player['Order'])
						mcts1 = mcts(iterationLimit=ITER_LIM)
	...
	for player in betting_temp_player_list:
		if one_player_left == True:
			...
			if player['Folded?'] == False:
				player_type_index = determine_player_type_indexes(player)
				if player['Player_Type'] != 'Human':
					initialState = PokerState(player['Name'], False, True, last_player_to_raise, amount_to_match, player['Order'])
					mcts1 = mcts(iterationLimit=ITER_LIM)
			...
			if player['Folded?'] == False:
				if last_player_to_raise == '':
					...
					if player['Name'] == last_player_to_raise:
						...
						if player['Player_Type'] != 'Human':
							initialState = PokerState(player['Name'], False, False, last_player_to_raise, amount_to_match, player['Order'])
							mcts1 = mcts(iterationLimit=ITER_LIM)
